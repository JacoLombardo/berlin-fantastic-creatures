...
function PersonalProfile() {
  ...
  const { user, getPersonalProfile, logout, checkIfUserIsLoggedIn, server } = useContext(AuthContext);
  ...
  const deleteAccount = async () => {
    ...
    try {
      const response = await fetch(`${server}/api/users/delete`, requestOptions);
      await response.json();
      logout();
    } catch (error) {
  ...
  const submitChange = async (profilePic, img_id) => {
    ...
    var requestOptions = { method: "POST", headers: myHeaders, body: urlencoded, redirect: "follow" };
    try {
      const response = await fetch(`${server}/api/users/update`, requestOptions);
